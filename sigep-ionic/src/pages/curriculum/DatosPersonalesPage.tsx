...
const DatosPersonalesPage: React.FC = () => {
  ...
  const obtenerUbicacionActual = async () => {
    try {
      ...
      // Opcional: usar geocodificación inversa (ejemplo con OpenStreetMap)
      // Por ahora solo mostramos coordenadas y las ponemos en dirección
      const direccion = `${latitude.toFixed(6)}, ${longitude.toFixed(6)}`;
      setContacto((prev) => ({ ...prev, direccionResidencia: direccion }));
      // Mostrar toast o alerta informativa
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await Haptics.vibrate(); // vibración en error también
      await LocalNotifications.schedule({
        notifications: [
          {
            title: "Error",
  ...
  // Renderizado con componentes Ionic
  return (
      <IonPage>
        <IonHeader>
          <IonToolbar>
