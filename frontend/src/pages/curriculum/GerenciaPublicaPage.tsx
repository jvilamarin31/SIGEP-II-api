...
// Componente para mostrar cada registro de forma detallada
const DetailItem: React.FC<{ item: Record<string, unknown>; type: string }> = ({ item, type }) => {
  ...
  return <div className="detail-item">{String(item.id ?? "Registro")}</div>;
};
...
const GerenciaPublicaPage: React.FC = () => {
  ...
  // Cargar el flag de persona expuesta políticamente
  useEffect(() => {
  ...
  // Si aún estamos determinando el flag, mostramos un loading
  if (personaExpuesta === null && loading) {
    return (
        <AppLayout title="Gerencia Pública">
          <div className="alert alert-info" style={{ marginBottom: 20 }}>Verificando permisos para acceder a esta sección...</div>
        </AppLayout>
    );
  }

  // Si no es persona expuesta políticamente, mostramos mensaje de bloqueo mejorado
  if (personaExpuesta !== true) {
    return (
        <AppLayout title="Gerencia Pública">
          <div className="page-header animate-in">
            <h2>Gerencia Pública</h2>
            <p>Información sobre publicaciones, premios, proyectos y participación en corporaciones.</p>
          </div>
          <div className="card" style={{ padding: 32, textAlign: "center", maxWidth: 600, margin: "0 auto" }}>
            <div style={{ fontSize: "3rem", marginBottom: 16 }}>🔒</div>
            <h3 style={{ marginBottom: 12 }}>Acceso restringido</h3>
            <p style={{ color: "var(--gray-600)", marginBottom: 20 }}>
              Para poder registrar información en esta sección debes marcar la casilla <strong>“Persona expuesta políticamente”</strong> en el formulario de <strong>Datos Personales → Datos Básicos</strong>.
            </p>
            <button className="btn btn-primary" onClick={() => window.location.href = "/curriculum/datos-personales"}>
  ...
  // Renderizado normal (con listas detalladas)
  return (
      <AppLayout title="Gerencia Pública">
        <div className="page-header animate-in">
          <h2>Gerencia Pública</h2>
