...
const DatosPersonalesPage: React.FC = () => {
  ...
  const [basicos, setBasicos] = useState({
    ...
    tieneLibretaMilitar: false,          // ← recuperado del commit viejo
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      ...
      const cargarBasicos = curriculumService.obtenerDatosBasicos()
          .then((data) => {
            // Determinar si tiene libreta basado en la existencia de algún dato de libreta
  ...
  const guardarBasicos = async () => {
    ...
    if (datosBasicosExistentes) {
      // Solo actualiza los campos que corresponden (el DTO de actualización tiene los mismos campos)
  ...
  return (
      <AppLayout title="Datos Personales">
        ...
        <form onSubmit={handleSave}>
          {activeTab === 0 && (
              <div className="animate-in">
                ...
                <div className="form-section">
                  ...
                  <div className="form-section-body">
                    {/* Checkbox para indicar si tiene libreta */}
                    <div className="form-checkbox-group" style={{ marginBottom: 16 }}>
                      <input
                          ...
                          onChange={e => setBasicos(p => ({
                            ...
                            // Limpiar campos de libreta si se desmarca
                    ...
                    {/* Campos de libreta: solo se muestran si tieneLibretaMilitar es true */}
                        ...
                        <>
                          <div className="form-grid cols-3">
                            ...
                            <div className="form-group">
                              ...
                              <input className="form-input" value={basicos.numeroLibretaMilitar} onChange={e => setBasicos(p => ({ ...p, numeroLibretaMilitar: e.target.value }))} />
                            </div>
                            <div className="form-group">
                              <label className="form-label">Distrito militar</label>
                              <input type="number" min={1} className="form-input" value={basicos.distritoMilitar} onChange={e => setBasicos(p => ({ ...p, distritoMilitar: e.target.value }))} />
                            </div>
                            <div className="form-group">
                              <FileUploadField
                    ...
                    <div className="form-checkbox-group" style={{ marginTop: 12 }}>
                      <input type="checkbox" id="pep" checked={basicos.personaExpuestaPoliticamente} onChange={e => setBasicos(p => ({ ...p, personaExpuestaPoliticamente: e.target.checked }))} />
                      <label htmlFor="pep">¿Persona expuesta políticamente?</label>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {/* Las demás secciones (Datos Demográficos y Contacto) se mantienen igual que en el commit "Validaciones" */}
          {activeTab === 1 && (
              <div className="animate-in">
                <div className="form-section">
